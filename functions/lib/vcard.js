...
exports.getVCard = functions.https.onRequest(async (req, res) => {
...
<p>El propietario de esta tarjeta ha desactivado su perfil momentáneamente.</p>
        ...
        if (identity.company)
            lines.push(`ORG:${identity.company}`);
        if (identity.role)
            lines.push(`TITLE:${identity.role}`);
        if (contact.mobile)
            lines.push(`TEL;TYPE=CELL:${contact.mobile}`);
        if (contact.landline)
            lines.push(`TEL;TYPE=WORK,VOICE:${contact.landline}`);
        if (contact.email)
            lines.push(`EMAIL;TYPE=WORK,INTERNET:${contact.email}`);
        if (contact.website)
            lines.push(`URL:${contact.website}`);
        if (context?.notes)
            lines.push(`NOTE:${context.notes.replace(/\\r\\n|\\r|\\n/g, ' ').trim()}`);
